...
public class ServiceApi:WebAPIHandler {
    ...
    func getData(url: String, withHeader header: [String : String]? = nil, withMethod method: URLMethod?,completion: @escaping onCompletion) {
        
        if let response = cache.getValue(for: url) {
            completion(response, nil)
        } else {
            task =   getDataFromServer(url: url, withHeader: header, withMethod: method) { [unowned self] (data, error) in
